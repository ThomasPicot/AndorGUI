...
class MainWindow(QMainWindow):
    ...
    def display_image_int(self):  
        ...
        if self.normalization_roi_visible:
            position_norm = np.array(self.normalization_roi.pos())
            size_norm = self.normalization_roi.size()
            image = np.array(self.andor.acquisition())
            roi_image = self.image_item.getProcessedImage().copy()
            roi_image = roi_image.crop(QRectF(position_norm[0], position_norm[1], size_norm[0], size_norm[1]))
            logging.debug("normalize image")
