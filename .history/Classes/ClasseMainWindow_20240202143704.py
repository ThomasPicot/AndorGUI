...
class MainWindow(QMainWindow):
    ...
    def toggle_cam_ROI(self):
        if self.cam_ROI is None:
            # Create ROI the first time the button is clicked
            self.cam_ROI = pg.RectROI([1020, 1020], [300, 300], pen=(0, 9))
            self.main_view.addItem(self.cam_ROI)
            self.cam_ROI.sigRegionChangeFinished.connect(self.roi_norm_changed)
        else:
            # Toggle ROI visibility
            self.cam_ROI.setVisible(not self.roi.isVisible())
            self.cam_ROI = None
